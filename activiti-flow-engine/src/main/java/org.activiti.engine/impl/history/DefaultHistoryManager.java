...
public class DefaultHistoryManager extends AbstractManager implements HistoryManager {
  ...
  public void recordProcessInstanceEnd(String processInstanceId, String deleteReason, String activityId) {
    ...
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      if (historicProcessInstance != null) {
        ...
        String approvalStatus = historicProcessInstance.getApprovalStatus();
        if (ApprovalStatus.APPROVALING.equals(approvalStatus)) {
          historicProcessInstance.setApprovalStatus(ApprovalStatus.FINISHED);
        }
        // Fire event
  ...
  @Override
  public void recordProcessInstanceEnd(String processInstanceId, String approvalStatus) {
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      HistoricProcessInstanceEntity historicProcessInstance = getHistoricProcessInstanceEntityManager().findById(processInstanceId);

      if (historicProcessInstance != null) {
        historicProcessInstance.setApprovalStatus(approvalStatus);
        // Fire event
        ActivitiEventDispatcher activitiEventDispatcher = getEventDispatcher();
        if (activitiEventDispatcher != null && activitiEventDispatcher.isEnabled()) {
          activitiEventDispatcher.dispatchEvent(
  ...
  @Override
  public void recordActivityStart(ExecutionEntity executionEntity, FlowElement flowElement) {
    HistoricActivityInstanceEntity historicActivityInstanceEntity = null;

    // Historic activity instance could have been created (but only in cache, never persisted)
    // for example when submitting form properties
    HistoricActivityInstanceEntity historicActivityInstanceEntityFromCache =
            getHistoricActivityInstanceFromCache(executionEntity.getId(), flowElement.getId(), true);
    if (historicActivityInstanceEntityFromCache != null) {
      historicActivityInstanceEntity = historicActivityInstanceEntityFromCache;
    } else {
      historicActivityInstanceEntity = createHistoricActivityInstanceEntity(executionEntity,flowElement);
    }
  ...
  public void recordActivityEnd(ExecutionEntity executionEntity, String deleteReason) {
    if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      if (historicActivityInstance != null) {
        ...
        // Fire event
       // historicActivityInstance.setTaskStatus(TaskStatus.AGREE);
        ActivitiEventDispatcher activitiEventDispatcher = getEventDispatcher();
        if (activitiEventDispatcher != null && activitiEventDispatcher.isEnabled()) {
          activitiEventDispatcher.dispatchEvent(
  ...
  protected HistoricActivityInstanceEntity createHistoricActivityInstanceEntity(ExecutionEntity execution,FlowElement flowElement) {
    ...
    historicActivityInstance.setActivityId(flowElement.getId());
    //historicActivityInstance.setTaskStatus();
      historicActivityInstance.setActivityName(flowElement.getName());
      String s = parseActivityType(flowElement);
      historicActivityInstance.setActivityType(s);
      if ("userTask".equals(s)) {
        historicActivityInstance.setTaskStatus(TaskStatus.APPROVALING);
      }
  ...
  protected HistoricActivityInstanceEntity createHistoricActivityInstanceEntity(ExecutionEntity execution) {
    ...
    if (execution.getCurrentFlowElement() != null) {
      ...
      String s = parseActivityType(execution.getCurrentFlowElement());
      historicActivityInstance.setActivityType(s);
      if ("userTask".equals(s)) {
        historicActivityInstance.setTaskStatus(TaskStatus.APPROVALING);
      }
  ...
  @Override
  public void recordTaskStatusChange(String taskId, String taskStatus) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      HistoricActivityInstanceEntityManager historicActivityInstanceEntityManager = getHistoricActivityInstanceEntityManager();
      HistoricTaskInstanceEntity historicTaskInstanceEntity = getHistoricTaskInstanceEntityManager().findById(taskId);
      if (historicTaskInstanceEntity != null) {
        historicTaskInstanceEntity.setTaskStatus(taskStatus);
        HistoricActivityInstanceEntity historicActivityInstanceEntity =
                findActivityInstance(getExecutionEntityManager().findById(historicTaskInstanceEntity.getExecutionId()),
                        historicTaskInstanceEntity.getTaskDefinitionKey(), false, false);
        if (historicActivityInstanceEntity != null) {
         // HistoricActivityInstanceEntity historicActivityInstanceEntity = historicActivityInstancesByTaskIdAndProcessInstanceId.get(0);
          historicActivityInstanceEntity.setTaskStatus(taskStatus);
  ...
  public void updateTaskStatus(String taskId, String taskStatus) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      ...
      if (historicTaskInstanceEntity != null) {
        historicTaskInstanceEntity.setTaskStatus(taskStatus);
        getHistoricTaskInstanceEntityManager().update(historicTaskInstanceEntity);
        HistoricActivityInstanceEntity historicActivityInstanceEntity =
                findActivityInstance(getExecutionEntityManager().findById(historicTaskInstanceEntity.getExecutionId()),
                        historicTaskInstanceEntity.getTaskDefinitionKey(), false, false);
        if (historicActivityInstanceEntity != null) {
  ...
  @Override
  public void recordProcessStatusChange(String processInstanceId, String approvalStatus) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      HistoricProcessInstanceEntityManager historicProcessInstanceEntityManager = getHistoricProcessInstanceEntityManager();
      HistoricProcessInstanceEntity processInstanceEntity = historicProcessInstanceEntityManager.findById(processInstanceId);
      if (processInstanceEntity != null) {
          processInstanceEntity.setApprovalStatus(approvalStatus);
          historicProcessInstanceEntityManager.update(processInstanceEntity);
      }
    }
  }
  ...
  public void recordTaskNameChange(String taskId, String taskName) {
    if (isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
      ...
      if (historicTaskInstance != null) {
        ...
        List<HistoricActivityInstanceEntity> list = getHistoricActivityInstanceEntityManager().findHistoricActivityInstancesByTaskIdAndProcessInstanceId(taskId, historicTaskInstance.getProcessInstanceId());
