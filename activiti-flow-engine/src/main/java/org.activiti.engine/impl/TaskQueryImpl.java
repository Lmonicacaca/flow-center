...
public class TaskQueryImpl extends AbstractVariableQueryImpl<TaskQuery, Task> implements TaskQuery {
  ...
  public TaskQuery taskWithoutTenantId() {
    ...
    @Override
    public TaskQuery taskStatus(List<String> taskStatus) {
        if (taskStatus == null) {
            throw new ActivitiIllegalArgumentException("taskStatus list is null");
        }

        if (taskStatus.isEmpty()) {
            throw new ActivitiIllegalArgumentException("taskStatus list is empty");
        }

        for (String status : taskStatus) {
            if (status == null) {
                throw new ActivitiIllegalArgumentException("None of the given task status can be null");
            }
        }

        if (this.taskStatus != null) {
            throw new ActivitiIllegalArgumentException("Invalid query usage: cannot set both taskStatus ");
        }

        if (orActive) {
            currentOrQueryObject.taskStatus = taskStatus;
        } else {
