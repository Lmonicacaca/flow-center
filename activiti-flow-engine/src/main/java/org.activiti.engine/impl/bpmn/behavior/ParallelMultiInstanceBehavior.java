...
public class ParallelMultiInstanceBehavior extends MultiInstanceActivityBehavior {
  ...
  public void leave(DelegateExecution execution) {
    ...
    if (executionEntity.getParent() != null) {
      ...
      if (nrOfCompletedInstances >= nrOfInstances || completionConditionSatisfied(execution.getParent())) {
        ...
        //修改其他taskStatus='approvaling'的或签任务状态orSignComplete
        updateOtherOrSignTaskStatus(executionToUse,execution.getId());
  ...
  protected void updateOtherOrSignTaskStatus(DelegateExecution execution,String executionId) {
    TaskService taskService = Context.getCommandContext().getProcessEngineConfiguration().getTaskService();
    List<Task> list = taskService.createTaskQuery().active().taskDefinitionKey(((ExecutionEntityImpl) execution).getActivityId()).processInstanceId(execution.getProcessInstanceId()).taskStatus(Lists.newArrayList("approvaling")).list();
    if (!CollectionUtils.isEmpty(list)) {
      List<Task> updateTaskList = Lists.newArrayList();
      for (Task task : list) {
      ...
      if (!CollectionUtils.isEmpty(updateTaskList)) {
        for (Task task : updateTaskList) {
          HistoryManager historyManager = Context.getCommandContext().getProcessEngineConfiguration().getHistoryManager();
          historyManager.recordTaskStatusChange(task.getId(), TaskStatus.OR_SIGN_FINISHED);
        }
      }
    }
  }
